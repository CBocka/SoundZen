package com.cbocka.soundzen.ui.mymusic.song_in_directory.adapter
...
class SongsInDirectoryAdapter(private val context : Context,
                              private val onClick : (Song, List<Song>) -> Unit,
                              private val onLongClick : (Song) -> Boolean)
    : ListAdapter<Song, SongsInDirectoryAdapter.SongsInDirectoryViewHolder>(SONG_COMPARATOR) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SongsInDirectoryViewHolder {
        val layoutInflater = LayoutInflater.from(parent.context)
        return SongsInDirectoryViewHolder(ItemAudioFileLayoutBinding.inflate(layoutInflater, parent, false))
    }

    override fun onBindViewHolder(holder: SongsInDirectoryViewHolder, position: Int) {
        val item = currentList[position]
        holder.bind(item)
    }

    inner class SongsInDirectoryViewHolder(private val binding : ItemAudioFileLayoutBinding) : RecyclerView.ViewHolder(binding.root) {
